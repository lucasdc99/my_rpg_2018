...
window_t *init_end(window_t *win)
{
    ...
    if (win->player->health <= 0) {
        ...
        init_text(&win->scene[END].text[0], "Mort", get_pos_float(905, 20));
    } else {
        ...
        init_text(&win->scene[END].text[0], "Victoire", get_pos_float(880, 20));
    }
    ...
    init_text(&win->scene[END].text[1], str, get_pos_float(905, 200));
    win->no_saves = 1;
