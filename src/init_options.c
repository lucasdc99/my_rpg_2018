...
window_t *init_options(window_t *win)
{
    ...
    get_pos_float(pos_window.x + 150, 10));
    init_text(&win->scene[OPTIONS].text[1], text[1],
    get_pos_float(pos_window.x, 210));
    init_text(&win->scene[OPTIONS].text[2], text[2],
    get_pos_float(pos_window.x, pos_window.y - 430));
    init_text(&win->scene[OPTIONS].text[3], text[3],
    get_pos_float(pos_window.x, pos_window.y - 230));
    init_text(&win->scene[OPTIONS].text[4], text[4],
    get_pos_float(pos_window.x + 400, pos_window.y - 250));
    ...
    set_next_buttons(&win->scene[OPTIONS].button[0], win->rect_buttons,
    order_button[0]);
    init_button(&win->scene[OPTIONS].button[0], pos_window, size,
    win->texture_button);
    set_next_buttons(&win->scene[OPTIONS].button[1], win->rect_buttons,
    order_button[1]);
    init_button(&win->scene[OPTIONS].button[1],
    get_pos_float(pos_window.x + 200, 200), get_pos_float(80, 80),
    win->texture_button);
    set_next_buttons(&win->scene[OPTIONS].button[2], win->rect_buttons,
    order_button[2]);
    init_button(&win->scene[OPTIONS].button[2],
    get_pos_float(pos_window.x + 400, 200), get_pos_float(80, 80),
    win->texture_button);
    set_next_buttons(&win->scene[OPTIONS].button[3], win->rect_buttons,
    order_button[3]);
    init_button(&win->scene[OPTIONS].button[3],
    get_pos_float(pos_window.x + 200, pos_window.y - 450),
    get_pos_float(100, 100), win->texture_button);
    set_next_buttons(&win->scene[OPTIONS].button[4], win->rect_buttons,
    order_button[4]);
    init_button(&win->scene[OPTIONS].button[4],
    get_pos_float(pos_window.x + 200, pos_window.y - 250),
    get_pos_float(150, 100), win->texture_button);
    if (win->fps == 60) {
