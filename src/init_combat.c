...
window_t *init_combat(window_t *win)
{
    ...
    init_button(&win->scene[COMBAT].button[1], get_pos_float(-200, -200), size, win->texture_button);
    set_next_buttons(&win->scene[COMBAT].button[2], win->rect_buttons, order_button[2]);
    init_button(&win->scene[COMBAT].button[2], get_pos_float(1600, 200), get_pos_float(100, 100), win->texture_button);
    set_next_buttons(&win->scene[COMBAT].button[3], win->rect_buttons, order_button[2]);
    init_button(&win->scene[COMBAT].button[3], get_pos_float(1600, 300), get_pos_float(100, 100), win->texture_button);
    set_next_buttons(&win->scene[COMBAT].button[4], win->rect_buttons, order_button[2]);
    init_button(&win->scene[COMBAT].button[4], get_pos_float(1600, 400), get_pos_float(100, 100), win->texture_button);
    sfSprite_setTextureRect(win->scene[COMBAT].sprite[0].sprite, get_rect(297, 56, 30, 30));
    sfSprite_setScale(win->scene[COMBAT].sprite[0].sprite, get_pos_float(4, 4));
    init_text(&win->scene[COMBAT].text[0], my_itc(win->player->health), get_pos_float(1500, 10));
    init_text(&win->scene[COMBAT].text[1], my_itc(win->player->strength), get_pos_float(1500, 100));
    init_enemy(win);
